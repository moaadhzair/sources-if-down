...
async function extractStreamUrl(url) {
    try {
        ...
        const voeGermanDub = finishedList.find(video => video.provider === 'VOE' && video.language === 'mit Untertitel Deutsch');
        if (voeGermanDub) {
            firstVideo = voeGermanDub;
        }
        else {
            const voeGerSub = finishedList.find(video => video.provider === 'VOE' && video.language === 'mit Untertitel Englisch');
            if (voeGerSub) {
